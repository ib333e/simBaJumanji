...
def setup_agent(cfg: DictConfig, env: Environment) -> Agent:
    ...
    if cfg.agent == "random":
    ...
    elif cfg.agent == "a2c_simba":
        actor_critic_networks = _setup_actor_critic_neworks_simba(cfg, env)
        optimizer = optax.adam(cfg.env.a2c.learning_rate)
        agent = A2CAgent(
            env=env,
            n_steps=cfg.env.training.n_steps,
        ...
        raise ValueError(f"Expected agent name to be in ['random', 'a2c', 'a2c_simba'], got {cfg.agent}.")
    return agent
