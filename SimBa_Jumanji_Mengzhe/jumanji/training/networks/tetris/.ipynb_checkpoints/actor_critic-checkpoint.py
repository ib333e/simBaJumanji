...
def make_network_cnn_simba(
    ...
    def residual_mlp_2d(x: jnp.ndarray) -> jnp.ndarray:
        """Applies ResidualMLP along the channel axis of a 4-D tensor."""
        b, h, w, c = x.shape
        x_flat = jnp.reshape(x, (b * h * w, c))     # [B·H·W, C]
        x_flat = ResidualMLP(c)(x_flat)             # channel-wise SimBa block
        return jnp.reshape(x_flat, (b, h, w, c))    # back to [B, H, W, C]
    ...
    def net(obs) -> chex.Array:
        # ---------- 1) grid branch  (UNCHANGED Conv tower, but RSNorm first)
        timestep = obs.step_count
        grid = obs.grid.astype(jnp.float32)[..., None]
        grid_norm = RSNorm(name="grid_norm")
        g = grid_norm(grid, timestep)
        g = hk.Conv2D(conv_num_channels, (3,5), (1,1), padding="SAME")(g)
        g = residual_mlp_2d(g)        
        g = jax.nn.relu(g)
        
        g = hk.Conv2D(conv_num_channels, (3,5), (2,1), padding="SAME")(g)
        g = residual_mlp_2d(g)        
        g = jax.nn.relu(g)
